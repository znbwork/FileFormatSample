''''''''''''''''''''''''''''''''''''''''''''''''''
' ConsolidateDataVertically
''''''''''''''''''''''''''''''''''''''''''''''''''
Sub ConsolidateDataVertically()
    ...
    Const TARGET_COLUMN As Long = 1 ' This is the column where all data will be pasted (1 = A, 2 = B, etc.)
    ...
    ' Initialize a row counter for the Summary sheet.
    rowCounter = 1

    ' Add a header to the summary sheet.
    summaryWs.Cells(rowCounter, TARGET_COLUMN).Value = "Consolidated Data"
    ...
    For i = 2 To lastSettingsRow
        ...
        For currentSourceCol = startCol To endCol
            If lastSourceRow > 1 Then
                ' Copy the data from the source sheet, starting from the second row to avoid headers.
                sourceWs.Range(sourceWs.Cells(2, currentSourceCol), sourceWs.Cells(lastSourceRow, currentSourceCol)).Copy

                ' Paste the data to the next available row on the Summary sheet.
                summaryWs.Cells(rowCounter, TARGET_COLUMN).PasteSpecial xlPasteValues

                ' Update the row counter to the next empty row.
                rowCounter = summaryWs.Cells(summaryWs.Rows.Count, TARGET_COLUMN).End(xlUp).Row + 1
            End If
...
NextIteration:
    ...
    summaryWs.Columns(TARGET_COLUMN).AutoFit
